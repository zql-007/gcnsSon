...
export default {
        ...
        tooltip: {
          ...
          formatter: params => {
            ...
            const matchItem = fullData.find(
              item => item.time === timeRange && item.thick === spec
            )
  ...
  mounted() {
    ...
    this.optionsMark = this.dataDictionary['tx-rawInvType']
    this.optionsType = this.dataDictionary['tx-rawMaterialType']
    this.finishOptionType = this.dataDictionary['tx-finishGoodsType']
    this.finishOptionMark = this.dataDictionary['tx-FGInvOccType']
  },
  methods: {
    changeMteriaMark(val) {
    ...
    async getStoreOfInitData() {
      const res = await post(getTxSourceStove, {
    ...
    async getFinishProductAll() {
      const res = await post(getTxProdStove, {
