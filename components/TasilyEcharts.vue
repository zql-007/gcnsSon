...
export default {
      ...
      if (newValue != oldValue) {
        setTimeout(() => {
          ...
          this.$nextTick(function() {
            ...
            if (!container) return // 校验 DOM 是否存在
            // 销毁旧实例（如果存在）
            if (vm.echart && vm.echart.dispose) {
              vm.echart.dispose()
            }
            ...
            // })
            // 替换匿名函数为具名函数
            window.removeEventListener('resize', vm.handleResize) // 先移除旧监听
  ...
  created() {
    ...
    if (window.__POWERED_BY_QIANKUN__) {
      window.addEventListener('unmount', this.handleQiankunUnmount)
    }
  },
  mounted() {
    let vm = this
    this.$nextTick(function() {
      ...
      // 强制销毁可能存在的旧实例（微前端环境下的双重保险）
      if (vm.echart && vm.echart.dispose) {
        vm.echart.dispose()
      }
      // 清除可能残留的全局事件监听（防止重复绑定）
      window.removeEventListener('resize', vm.handleResize)

      // 重新初始化实例
      vm.echart = Echarts.init(container, themeEcharts)
      // 绑定事件和设置配置
      if (vm.group) vm.handlerConnect(vm.group)
      vm.echart.setOption(vm.getDefaultOption())
      // 绑定点击事件（注意使用箭头函数保留 this 指向）
      vm.echart.on('click', params => {
        vm.$emit('chart-click-data', {
          data: params,
          id: vm.$el.id,
          echart: vm.echart
      ...
      vm.echart.on('legendselectchanged', params => {
        vm.$emit('chart-legendselectchanged', {
          data: params,
          id: vm.$el.id,
          echart: vm.echart
  ...
  // 在组件的 methods 同级添加销毁钩子
  beforeDestroy() {
    console.log('销毁 ECharts 实例')
    // 1. 销毁 ECharts 实例（关键）
    if (this.echart && typeof this.echart.dispose === 'function') {
      this.echart.dispose()
      this.echart = null // 置空避免残留引用
    }
    // 2. 移除全局事件监听（使用具名函数确保能正确移除）
    window.removeEventListener('resize', this.handleResize)
    // 3. 清除 DOM 引用（可选，针对微前端环境）
    ...
    // 移除 qiankun 事件监听
    if (window.__POWERED_BY_QIANKUN__) {
      window.removeEventListener('unmount', this.handleQiankunUnmount)
    }
  },
  methods: {
    handleResize() {
